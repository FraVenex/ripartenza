...
async function run() {
  ...
  console.log('Logging in to Garmin for', email);
  const gc = new GarminConnect({ username: email, password });
  await gc.login();

  console.log('Login successful! Fetching activities from Garmin API...');
  const activities = await gc.client.get(
    'https://connectapi.garmin.com/activitylist-service/activities/search/activities',
    { params: { start: 0, limit: 15, activityType: 'running' } }
  );

  console.log(`Fetched ${activities.length} running activities from Garmin`);
  ...
  for (const act of activities) {
    ...
    if (err) console.error('Error saving', activityDate, err);
    else {
      console.log(
        `REAL Garmin Activity: ${activityDate} | ${act.activityName} | Dist: ${distanceM}m | Dur: ${durationS}s (${Math.round(durationS / 60)} min) | HR avg: ${avgHr} / max: ${maxHr} | Pace: ${avgPaceMinKm?.toFixed(2)} min/km`
      );
    }
  }
